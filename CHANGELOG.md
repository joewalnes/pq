...
- Delete `write_output::names_an_open_descriptor`, a compensation for the bug
  above that bypassed staging for `cat -O`/`jq -o` only, one layer above
  where the bug actually lived — every other `-o` command was still broken.
  One classifier now, in `output_guard` itself, covers all of them.
...
- Buffer `export`/`cat`'s file and stdout write paths (`export.rs`, `write_output.rs`, `output/mod.rs`'s `render_batches`), which previously wrote one syscall per JSON/JSONL row via a raw `File`/locked `Stdout`. Measured with `/usr/bin/time -l` (instructions retired + wall clock) on a 2M-row/268MB fixture: `export -o out.jsonl` dropped from a mean of ~163s (3 runs: 118s/196s/175s, dominated by 90-130s of *sys* time) to ~0.87s (5 runs, spread 0.86-0.88s) — output byte-identical (md5) across all 8 runs. `cat -f jsonl` redirected to a file: 5.99s → 1.16s. Every buffered writer flushes explicitly before returning, via a new shared `write_output::write_buffered` helper, so the flush always lands before `pq_transform::output_guard::with_atomic_output`'s rename — proven with a guard (`write_buffered_propagates_a_flush_error`) that fails if that flush's `Result` is ever swallowed, plus integration tests asserting no truncation past the buffer size and no destination corruption when a multi-file export fails partway through
...
- Fix `pq cat -f jsonl` (and every JSON-shaped output: `jq`, `grep`, `sql`, `export -f json/jsonl`) casting an entire dictionary-encoded column **once per row** instead of once per batch — quadratic in the batch's row count (default batch size 8192). Measured with `/usr/bin/time -l`, 3 runs each, on real dictionary-encoded parquet fixtures: a 50k-row dictionary column dropped from 27.9B to 467M instructions retired (~60x) and 1.23s to 0.03s wall time (~41x); a 20k-row column, ~47x/~46x; a 5k-row (single-batch) column, ~19x/~16x — scaling consistent with the per-row full-column cast hypothesis. Output unchanged
