...
This means `pq count` on a 925 MB file transfers under 600 KB. Use `--debug`
...
Data commands like `head` and `sql` then fetch only the row groups and
columns they need — never the entire file.
...
## Example: Overture Maps (Azure Blob Storage via HTTPS)

Overture Maps publishes a global places dataset as Parquet on Azure Blob
Storage. Individual files are 900+ MB with millions of rows and deeply nested
schemas (structs, lists, maps).
...
│   ├── xmin: float32 (nullable)
│   ├── xmax: float32 (nullable)
│   ├── ymin: float32 (nullable)
│   └── ymax: float32 (nullable)
├── version: int32 (nullable)
├── sources: list<struct> (nullable)
...
│   └── between: list<float64> (nullable)
├── names: struct (nullable)
│   ├── primary: string (nullable)
│   ├── common: map (nullable)
│   │   ├── key: string
│   │   └── value: string (nullable)
│   └── rules: list<struct> (nullable)
│       ├── variant: string (nullable)
│       ├── language: string (nullable)
│       ├── perspectives: struct (nullable)
│       │   ├── mode: string (nullable)
│       │   └── countries: list<string> (nullable)
│       ├── value: string (nullable)
│       ├── between: list<float64> (nullable)
│       └── side: string (nullable)
├── categories: struct (nullable)
│   ├── primary: string (nullable)
│   └── alternate: list<string> (nullable)
├── basic_category: string (nullable)
├── taxonomy: struct (nullable)
│   ├── primary: string (nullable)
│   ├── hierarchy: list<string> (nullable)
│   └── alternates: list<string> (nullable)
├── confidence: float64 (nullable)
├── websites: list<string> (nullable)
├── socials: list<string> (nullable)
├── emails: list<string> (nullable)
├── phones: list<string> (nullable)
├── brand: struct (nullable)
│   ├── wikidata: string (nullable)
│   └── names: struct (nullable)
│       ├── primary: string (nullable)
│       ├── common: map (nullable)
│       │   ├── key: string
...
│           ├── between: list<float64> (nullable)
│           └── side: string (nullable)
├── addresses: list<struct> (nullable)
│   ├── freeform: string (nullable)
...
```console
$ pq cat "https://overturemapswestus2.blob.core.windows.net/release/2026-02-18.0/theme=places/type=place/part-00000-308cb36d-c529-4dc2-83bb-fe6b282a2b1a-c000.zstd.parquet" --jq '{name: .names.primary, category: .categories.primary, city: .addresses[0].locality, country: .addresses[0].country}' --limit 5
```
